...
public class CrawlerMain {
    public static void main(String[] args) throws IOException {
        if (args.length < 2) {
            System.out.println("Usage: Crawler <rawQueryDataFilePath> <adsDataFilePath> "
                             + "<proxyFilePath> <logFilePath>");
            System.exit(0);
        }
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(rawQueryDataFilePath))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                List<Ad> ads = crawler.getAdBasicInfoByQuery(query, bidPrice, campaignId,
                                queryGroupId);
                for (Ad ad : ads) {
